import { Component, OnInit, Input } from '@angular/core';
import { ImageApiServiceService } from 'src/app/services/imageServices/image-api-service.service';
import { Image, QrModel, ResrvationCommand, Restaurant, SubMenuItem } from '../../models/models';
import { ActivatedRoute, Router } from '@angular/router';
import { CountryApiService } from 'src/app/services/CountryService/country-api.service';
import { ReservationApiServiceService } from 'src/app/services/reservationService/reservation-api-service.service';
import { DomSanitizer } from '@angular/platform-browser';
import { ReservationResponse } from '../../models/models';
import { FormBuilder, FormGroup } from '@angular/forms';
import { AuthService } from 'src/app/auth/services/auth.service';

@Component({
  selector: 'app-restaurant-booking',
  templateUrl: './restaurant-booking.component.html',
  styleUrls: ['./restaurant-booking.component.scss']
})

export class RestaurantBookingComponent implements OnInit {
  restaurantId: number;
  restaurantName: string;
  peopleCount: number = 15;
  // @Input()
  isBookDateSelected: boolean = false;
  isBookTimeSelected: boolean = false;
  isPersonCountSelected: boolean = false;
  bookDate: Date;

  isTableAccordionActive: boolean = false;
  isMenuAccordionActive: boolean = false;

  image: Image;
  selectedCountry: string;
  firstName: string;
  lastName: string;
  email: string;
  phoneNumber: number;
  specialOccasion: string;
  specialRequest: string;
  specialOccasions: any[];
  singMeUp: boolean = false;
  getRemainder: boolean = false;
  countries: any[];
  checked: boolean = false;
  reservationResponse: ReservationResponse | any;
  qrModel: QrModel | any;
  displayModal: boolean = false;
  timeNow: string = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
  today: string = new Date().toLocaleDateString("es-CL");
  minDate: Date = new Date()
  isUserAuthenticated: boolean = false;
  userName: string = '';
  surName: string = '';

  addInCallendar: boolean;
  bookingForm: FormGroup | any;

  constructor(private imageApiService: ImageApiServiceService, private countryApiService: CountryApiService,
    private route: ActivatedRoute, private reservationService: ReservationApiServiceService, private fb: FormBuilder,
    private sanitizer: DomSanitizer, private router: Router, public authService: AuthService) {
  }

  ngOnInit(): void {

    this.loadRestaurantId();
    this.loadRestaurantName();
    this.loadDataFromQuery();

    this.bookingForm = this.fb.group({
      restaurantId: [this.restaurantId],
      date: Date,
      time: [],
      tableId: Number,
      selectedMenu: [],
      getRemainder: Boolean,
      isUserLogIn: Boolean,
      singMeUp: Boolean,
      specialOccasion: String,
      specialRequest: String,
      guestUser: Object,
      addBookingToCallendar: Boolean,
    });


    this.specialOccasions = [{ name: "Birthday" }, { name: "Anniversary" }, { name: "Date night" }, { name: "Business Meal" }, { name: "Celebration" }];

    this.loadRestaurantTopImage();
    // this.loadCountries();
  }

  loadUserIsAuthenticated(): boolean {
    const getName = localStorage.getItem('name');
    this.userName = getName ? getName : '';
    const getSurName = localStorage.getItem('surName');
    this.surName = getSurName ? getSurName : '';

    this.isUserAuthenticated = this.authService.isAuthenticated();
    return this.isUserAuthenticated;
  }

  moveToSignInPage() {
    sessionStorage.setItem('bookingFormValue', JSON.stringify(this.bookingForm.value))

    var index = this.router.url.indexOf("?");
    let url;
    if (index > -1) {
      url = this.router.url.substr(0, index)
    }
    else
      url = this.router.url;

    this.router.navigate(
      ['/auth/login'],
      { queryParams: { redirectUri: url } }
    );
  }


  loadRestaurantTopImage() {
    this.imageApiService.GetRestaurantTopImage(this.restaurantId).subscribe((i: any) => {
      this.image = i;
    });
  }

  loadCountries() {
    this.countryApiService.ListOfCountry().subscribe((response: any) => {
      console.log(response);

      this.countries = response;
    })
  }


  onTableChoosed(tableId: number) {
    this.bookingForm.value.tableId = tableId;
    this.isTableAccordionActive = false;
  }

  onMenuChoosed(subMenus: SubMenuItem[]) {
    this.bookingForm.value.selectedMenu = subMenus;
  }

  Book() {
    if (!this.authService.isAuthenticated())
      this.bookingForm.value.guestUser = {
        'firstName': this.firstName,
        'lastName': this.lastName,
        'email': this.email,
        'phoneNumber': this.phoneNumber
      }

    this.bookingForm.value.specialRequest = this.specialRequest;
    this.bookingForm.value.specialOccasion = this.specialOccasion;
    this.bookingForm.value.singMeUp = this.singMeUp;
    this.bookingForm.value.getRemainder = this.getRemainder;
    this.bookingForm.value.addBookingToCallendar = this.addInCallendar;
    this.bookingForm.value.isUserLogIn = this.authService.isAuthenticated();

    console.log(this.bookingForm.value);


    this.reservationService.Reservation(this.bookingForm.value).subscribe(data => {
      this.reservationResponse = data;
      this.qrModel = this.reservationResponse.qrCode;
    })
    // this.showQrCode();
    // this.qrModel = { "qrImage": "asf", "fileName": "asdf" };

    // this.qrModel.qrImage = "data:image/png;base64,"
    // this.qrModel.fileName = "ReservationQrCode"

  }

  showQrCode() {
    console.log("asfdasf");

    this.displayModal = true;
  }

  downloadQRImage() {
    const downloadLink = document.createElement('a');

    downloadLink.href = this.qrModel.qrIamge;
    downloadLink.download = this.qrModel.fileName;
    downloadLink.click();
  }
  handleChange(e: { checked: any; }) {
    console.log(e);

    var isChecked = e.checked;
  }


  private loadRestaurantId() {
    var id = this.route.snapshot.paramMap.get("restaurantId");
    if (id != null) {
      this.restaurantId = parseInt(id);
    }
  }

  private loadRestaurantName() {
    this.route.queryParams
      .subscribe(params => {
        if (params.restaurantName)
          this.restaurantName = params.restaurantName;
      });
  }

  private loadDataFromQuery() {
    this.route.queryParams
      .subscribe(params => {
        if (params.date) {
          this.bookingForm.value.date = new Date(params.date).toDateString();
          this.isBookDateSelected = true;

          if (params.time) {
            this.bookingForm.value.time = new Date(params.date + params.time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
            this.isBookTimeSelected = true;
          }
        }
        if (params.personCount) {
          this.bookingForm.value.personCount = params.personCount;
          this.isPersonCountSelected = true;
        }

      }
      );
  }
}
